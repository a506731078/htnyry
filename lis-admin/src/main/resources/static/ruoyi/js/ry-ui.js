...
(function ($) {
    $.extend({
        ...
        table: {
            ...
            // 下载模板
            importTemplate2: function() {
                $.get(activeWindow().table.options.importTemplateUrl2, function(result) {
                    if (result.code == web_status.SUCCESS) {
                        window.location.href = ctx + "common/download?fileName=" + encodeURI(result.msg) + "&delete=" + true;
                    } else if (result.code == web_status.WARNING) {
                        $.modal.alertWarning(result.msg)
            ...
            // 导入数据
            importExcel2: function(formId, width, height) {
                table.set();
        ...
        operate: {
            ...
            edit2: function(id) {
                ...
                var url = table.options.createLocationAddUrl.replace("{id}", rows);
                $.modal.open("多选订单并定位" + table.options.modalName, url,500,500);
