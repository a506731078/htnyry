...
项目已全面支持 Docker 化部署，提供两种方案：

1.  **包含 MySQL 数据库**：Docker Compose 会同时启动 LIS 应用和 MySQL 数据库。
2.  **不包含 MySQL 数据库**：适用于连接外部已有的 MySQL 数据库。

**详细部署指南请查看：** [飞牛NAS部署指南.md](飞牛NAS部署指南.md)
...
#### 2. 选择合适的 `docker-compose.yml` 文件

根据你的数据库情况选择以下文件之一：

-   **有数据库**：使用 `docker-compose-with-mysql.yml`
-   **无数据库**：使用 `docker-compose-without-mysql.yml`

#### 3. 配置 `config.json` 及环境变量（如果需要）

确保在 `docker-compose.yml` 同目录下创建 `config.json` 文件，并配置正确的数据库连接信息。

#### 4. 启动服务

以不含数据库为例，在 `docker-compose-without-mysql.yml` 所在的目录下执行：
...
根据你的 `docker-compose.yml` 配置，访问相应的 IP 地址和端口（例如：`http://你的NAS_IP:5555`）。
...
**更多详细的部署步骤、配置示例、故障排查和高级操作，请查阅 [飞牛NAS部署指南.md](飞牛NAS部署指南.md) 文件。**
